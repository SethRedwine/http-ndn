...
public class TlvEncoder {
  ...
  /**
   * Create a new TlvEncoder with a default DynamicByteBuffer. When done, you
   * should call getOutput().
   */
  public TlvEncoder() {
   ...
   *               so this does not support encoding a 64-bit length.
   */
  public final void writeTypeAndLength(int type, int length) {
    // TODO: see if this works, selectors erroring at nfd
    if (type != 9) {
      // Write backwards.
   ...
   *              a Java 64-bit long, so encoding of 64-bit values is supported
   *              (actually 63-bit because a Java long is signed).
   */
  ...
  /**
   * Write the buffer from its position() to limit() to the output just before
  ...
  public final void writeBuffer(ByteBuffer buffer) {
    ...
    // Write backwards.
    int position = output_.setRemainingFromBack(output_.remaining() + buffer.remaining());
   ...
   *              buffer with the bytes of the blob.
   */
