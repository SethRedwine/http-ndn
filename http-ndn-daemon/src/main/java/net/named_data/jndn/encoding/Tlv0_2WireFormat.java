...
public class Tlv0_2WireFormat extends WireFormat {
   ...
   *              false, then Blob values share memory with the input, which must
   ...
   *                                 portion starts from the first name component
   *                                 and ends just before the final name component
   *                                 (which is assumed to be a signature for a
   *                                 signed interest).
   * @param signedPortionEndOffset   Return the offset in the encoding of the end
   *                                 of the signed portion. The signed portion
   *                                 starts from the first name component and ends
   *                                 just before the final name component (which
   *                                 is assumed to be a signature for a signed
   *                                 interest).
   * @return A Blob containing the encoding.
   */
  public Blob encodeInterest(Interest interest, int[] signedPortionBeginOffset, int[] signedPortionEndOffset) {
    if (!interest.getDidSetCanBePrefix_() && !didCanBePrefixWarning_) {
      System.out
          .println("WARNING: The default CanBePrefix will change. See Interest.setDefaultCanBePrefix() for details.");
    ...
    // throw new Error
    // ("An Interest may not have a selected delegation when encoding a forwarding
    // hint");
    // if (interest.hasLink())
    // throw new Error
    ...
    // return new Blob(encoder.getOutput(), false);
    return encodeInterestV03(interest, signedPortionBeginOffset, signedPortionEndOffset);
  }
   ...
   *                                 is assumed to be a signature for a signed
   *                                 interest).
   * @param copy                     If true, copy from the input when making new
   *                                 Blob values. If false, then Blob values share
   *                                 memory with the input, which must remain
   *                                 unchanged while the Blob values are used.
   * @throws EncodingException For invalid encoding.
   */
  public void decodeInterest(Interest interest, ByteBuffer input, int[] signedPortionBeginOffset,
      int[] signedPortionEndOffset, boolean copy) throws EncodingException {
    try {
      decodeInterestV02(interest, input, signedPortionBeginOffset, signedPortionEndOffset, copy);
    } catch (Exception exceptionV02) {
      try {
        // Failed to decode as format v0.2. Try to decode as v0.3.
        decodeInterestV03(interest, input, signedPortionBeginOffset, signedPortionEndOffset, copy);
      } catch (Exception ex) {
   ...
   */
  private void decodeInterestV02(Interest interest, ByteBuffer input, int[] signedPortionBeginOffset,
      int[] signedPortionEndOffset, boolean copy) throws EncodingException {
    TlvDecoder decoder = new TlvDecoder(input);

    int endOffset = decoder.readNestedTlvsStart(Tlv.Interest);
    decodeName(interest.getName(), signedPortionBeginOffset, signedPortionEndOffset, decoder, copy);
    ...
    interest
        .setInterestLifetimeMilliseconds(decoder.readOptionalNonNegativeIntegerTlv(Tlv.InterestLifetime, endOffset));

    if (decoder.peekType(Tlv.ForwardingHint, endOffset)) {
      int forwardingHintEndOffset = decoder.readNestedTlvsStart(Tlv.ForwardingHint);
      decodeDelegationSet(interest.getForwardingHint(), forwardingHintEndOffset, decoder, copy);
    ...
    if (decoder.peekType(Tlv.Data, endOffset)) {
      ...
      interest.unsetLink();
    interest
        .setSelectedDelegationIndex((int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.SelectedDelegation, endOffset));
   ...
   *                                 signedPortionBeginOffset[0]. If you are not
   *                                 encoding in order to sign, you can call
   *                                 encodeData(data) to ignore this returned
   *                                 value.
   * @param signedPortionEndOffset   Return the offset in the encoding of the end
   *                                 of the signed portion by setting
   *                                 signedPortionEndOffset[0]. If you are not
   *                                 encoding in order to sign, you can call
   *                                 encodeData(data) to ignore this returned
   *                                 value.
   * @return A Blob containing the encoding.
   ...
   *                                 beginning of the signed portion by setting
   *                                 signedPortionBeginOffset[0]. If you are not
   *                                 decoding in order to verify, you can call
   *                                 decodeData(data, input) to ignore this
   *                                 returned value.
   * @param signedPortionEndOffset   Return the offset in the input buffer of the
   *                                 end of the signed portion by setting
   *                                 signedPortionEndOffset[0]. If you are not
   *                                 decoding in order to verify, you can call
   *                                 decodeData(data, input) to ignore this
   *                                 returned value.
   ...
   * @throws EncodingException For invalid encoding.
   */
  public void decodeData(Data data, ByteBuffer input, int[] signedPortionBeginOffset, int[] signedPortionEndOffset,
      boolean copy) throws EncodingException {
    TlvDecoder decoder = new TlvDecoder(input);
    ...
    data.getSignature().setSignature(new Blob(decoder.readBlobTlv(Tlv.SignatureValue), copy));
   ...
   *                          position.
   * @param copy              If true, copy from the input when making new Blob
   *                          values. If false, then Blob values share memory with
   *                          the input, which must remain unchanged while the
   *                          Blob values are used.
   * @throws EncodingException For invalid encoding
   */
  public void decodeControlParameters(ControlParameters controlParameters, ByteBuffer input, boolean copy)
   ...
   * @param copy            If true, copy from the input when making new Blob
   *                        values. If false, then Blob values share memory with
   *                        the input, which must remain unchanged while the Blob
   *                        values are used.
   * @throws EncodingException For invalid encoding
   */
  public void decodeControlResponse(ControlResponse controlResponse, ByteBuffer input, boolean copy)
      throws EncodingException {
    TlvDecoder decoder = new TlvDecoder(input);
    ...
    Blob statusText = new Blob(decoder.readBlobTlv(Tlv.NfdCommand_StatusText), false);
    ...
    if (decoder.peekType(Tlv.ControlParameters_ControlParameters, endOffset)) {
      ...
      decodeControlParameters(controlResponse.getBodyAsControlParameters(), decoder, copy);
    } else
   ...
   * @throws EncodingException For invalid encoding.
   */
  public Signature decodeSignatureInfoAndValue(ByteBuffer signatureInfo, ByteBuffer signatureValue, boolean copy)
      throws EncodingException {
    ...
    signatureHolder.getSignature().setSignature(new Blob(decoder.readBlobTlv(Tlv.SignatureValue), copy));
  ...
  public void decodeLpPacket(LpPacket lpPacket, ByteBuffer input, boolean copy) throws EncodingException {
    ...
    while (decoder.getOffset() < endOffset) {
      ...
      if (fieldType == Tlv.LpPacket_Fragment) {
        ...
        int code = (int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.LpPacket_NackReason, fieldEndOffset);
        ...
        boolean canIgnore = (fieldType >= Tlv.LpPacket_IGNORE_MIN && fieldType <= Tlv.LpPacket_IGNORE_MAX
            && (fieldType & 0x03) == 0);
        if (!canIgnore)
   ...
   *                      to limit(), but does not change the position.
   * @param copy          If true, copy from the input when making new Blob
   *                      values. If false, then Blob values share memory with the
   *                      input, which must remain unchanged while the Blob values
   *                      are used.
   * @throws EncodingException For invalid encoding.
   */
  public void decodeDelegationSet(DelegationSet delegationSet, ByteBuffer input, boolean copy)
      throws EncodingException {
    TlvDecoder decoder = new TlvDecoder(input);
  ...
  public Blob encodeEncryptedContent(EncryptedContent encryptedContent) {
    ...
    encoder.writeNonNegativeIntegerTlv(Tlv.Encrypt_EncryptionAlgorithm,
        encryptedContent.getAlgorithmType().getNumericType());
    Tlv0_2WireFormat.encodeKeyLocator(Tlv.KeyLocator, encryptedContent.getKeyLocator(), encoder);
   ...
   * @throws EncodingException For invalid encoding
   */
  public void decodeEncryptedContent(EncryptedContent encryptedContent, ByteBuffer input, boolean copy)
      throws EncodingException {
    TlvDecoder decoder = new TlvDecoder(input);
    int endOffset = decoder.readNestedTlvsStart(Tlv.Encrypt_EncryptedContent);

    encryptedContent.clear();
    Tlv0_2WireFormat.decodeKeyLocator(Tlv.KeyLocator, encryptedContent.getKeyLocator(), decoder, copy);
    ...
    encryptedContent
        .setInitialVector(new Blob(decoder.readOptionalBlobTlv(Tlv.Encrypt_InitialVector, endOffset), copy));
    encryptedContent.setPayload(new Blob(decoder.readBlobTlv(Tlv.Encrypt_EncryptedPayload), copy));
  ...
  public Blob encodeEncryptedContentV2(EncryptedContent encryptedContent) {
    ...
    encoder.writeOptionalBlobTlv(Tlv.Encrypt_EncryptedPayloadKey, encryptedContent.getPayloadKey().buf());
    encoder.writeOptionalBlobTlv(Tlv.Encrypt_InitialVector, encryptedContent.getInitialVector().buf());
    encoder.writeBlobTlv(Tlv.Encrypt_EncryptedPayload, encryptedContent.getPayload().buf());
   ...
   * @throws EncodingException For invalid encoding.
   */
  public void decodeEncryptedContentV2(EncryptedContent encryptedContent, ByteBuffer input, boolean copy)
      throws EncodingException {
    TlvDecoder decoder = new TlvDecoder(input);
    int endOffset = decoder.readNestedTlvsStart(Tlv.Encrypt_EncryptedContent);
    ...
    encryptedContent
        .setInitialVector(new Blob(decoder.readOptionalBlobTlv(Tlv.Encrypt_InitialVector, endOffset), copy));
    encryptedContent
        .setPayloadKey(new Blob(decoder.readOptionalBlobTlv(Tlv.Encrypt_EncryptedPayloadKey, endOffset), copy));

    if (decoder.peekType(Tlv.Name, endOffset)) {
      decodeName(encryptedContent.getKeyLocator().getKeyName(), new int[1], new int[1], decoder, copy);
   ...
   * @param encoder                  The TlvEncoder to receive the encoding.
   */
  public static void encodeName(Name name, int[] signedPortionBeginOffset, int[] signedPortionEndOffset,
      TlvEncoder encoder) {
    int saveLength = encoder.getLength();
   ...
   *                                 and ends just before the final name component
   *                                 (which is assumed to be a signature for a
   *                                 signed interest). If you are not decoding in
   *                                 order to verify, you can ignore this returned
   *                                 value.
   * @param signedPortionEndOffset   Return the offset in the encoding of the end
   *                                 of the signed portion. The signed portion
   *                                 starts from the first name component and ends
   *                                 just before the final name component (which
   *                                 is assumed to be a signature for a signed
   *                                 interest). If you are not decoding in order
   *                                 to verify, you can ignore this returned
   *                                 value.
   * @param decoder                  The decoder with the input to decode.
   * @param copy                     If true, copy from the input when making new
   *                                 Blob values. If false, then Blob values share
   ...
   * @throws EncodingException
   */
  public static void decodeName(Name name, int[] signedPortionBeginOffset, int[] signedPortionEndOffset,
  ...
  private static void decodeSelectors(Interest interest, TlvDecoder decoder, boolean copy) throws EncodingException {
    ...
    interest
        .setMinSuffixComponents((int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.MinSuffixComponents, endOffset));
    interest
        .setMaxSuffixComponents((int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.MaxSuffixComponents, endOffset));

    if (decoder.peekType(Tlv.PublisherPublicKeyLocator, endOffset))
      decodeKeyLocator(Tlv.PublisherPublicKeyLocator, interest.getKeyLocator(), decoder, copy);
    else
      interest.getKeyLocator().clear();
  ...
  private static void decodeKeyLocator(int expectedType, KeyLocator keyLocator, TlvDecoder decoder, boolean copy)
      throws EncodingException {
    ...
    if (decoder.peekType(Tlv.Name, endOffset)) {
      ...
      keyLocator.setKeyData(new Blob(decoder.readBlobTlv(Tlv.KeyLocatorDigest), copy));
    } else
  ...
  private static void encodeValidityPeriod(ValidityPeriod validityPeriod, TlvEncoder encoder) {
    ...
    encoder.writeTypeAndLength(Tlv.ValidityPeriod_ValidityPeriod, encoder.getLength() - saveLength);
  }
  ...
  private static void encodeSignatureInfo(Signature signature, TlvEncoder encoder) {
    ...
    if (signature instanceof Sha256WithRsaSignature) {
      if (((Sha256WithRsaSignature) signature).getValidityPeriod().hasPeriod())
        encodeValidityPeriod(((Sha256WithRsaSignature) signature).getValidityPeriod(), encoder);
      ...
      if (((Sha256WithEcdsaSignature) signature).getValidityPeriod().hasPeriod())
        encodeValidityPeriod(((Sha256WithEcdsaSignature) signature).getValidityPeriod(), encoder);
      ...
      encodeKeyLocator(Tlv.KeyLocator, ((HmacWithSha256Signature) signature).getKeyLocator(), encoder);
  ...
  private static void decodeSignatureInfo(SignatureHolder signatureHolder, TlvDecoder decoder, boolean copy)
      throws EncodingException {
    ...
    if (signatureType == Tlv.SignatureType_SignatureSha256WithRsa) {
      ...
      decodeKeyLocator(Tlv.KeyLocator, signatureInfo.getKeyLocator(), decoder, copy);
  ...
  private static void encodeMetaInfo(MetaInfo metaInfo, TlvEncoder encoder) {
    ...
    if (!(metaInfo.getType() == ContentType.BLOB)) {
      ...
      if (metaInfo.getType() == ContentType.LINK || metaInfo.getType() == ContentType.KEY
        ...
        encoder.writeNonNegativeIntegerTlv(Tlv.ContentType, metaInfo.getOtherTypeCode());
      else
  ...
  private static void decodeControlParameters(ControlParameters controlParameters, TlvDecoder decoder, boolean copy)
      throws EncodingException {
    ...
    // decode face ID
    controlParameters
        .setFaceId((int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.ControlParameters_FaceId, endOffset));

    // decode URI
    if (decoder.peekType(Tlv.ControlParameters_Uri, endOffset)) {
      // Set copy false since we just immediately get the string.
      Blob uri = new Blob(decoder.readOptionalBlobTlv(Tlv.ControlParameters_Uri, endOffset), false);
    ...
    controlParameters.setLocalControlFeature(
        (int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.ControlParameters_LocalControlFeature, endOffset));
    controlParameters
        .setOrigin((int) decoder.readOptionalNonNegativeIntegerTlv(Tlv.ControlParameters_Origin, endOffset));
    ...
    decoder.skipOptionalTlv(Tlv.ControlParameters_BaseCongestionMarkingInterval, endOffset);
    decoder.skipOptionalTlv(Tlv.ControlParameters_DefaultCongestionThreshold, endOffset);
    ...
    if (decoder.peekType(Tlv.ControlParameters_Strategy, endOffset)) {
      ...
      decodeName(controlParameters.getStrategy(), new int[1], new int[1], decoder, copy);
      decoder.finishNestedTlvs(strategyEndOffset);
    }

    // decode expiration period
    controlParameters.setExpirationPeriod(
        decoder.readOptionalNonNegativeIntegerTlv(Tlv.ControlParameters_ExpirationPeriod, endOffset));
  ...
  /**
   * Encode delegationSet to the encoder as a sequence of NDN-TLV Delegation. Note
   ...
   * @throws EncodingException For invalid encoding.
   */
  private static void decodeDelegationSet(DelegationSet delegationSet, int endOffset, TlvDecoder decoder, boolean copy)
      throws EncodingException {
    delegationSet.clear();
    while (decoder.getOffset() < endOffset) {
   ...
   * @return A Blob containing the encoding.
   */
  private static Blob encodeInterestV03(Interest interest, int[] signedPortionBeginOffset,
      int[] signedPortionEndOffset) {
    // TODO: Throw an exception if the interest speficies V02 fields.
    // TODO: see if this works
    // System.out.println("Encoding v0.3 packet...");

    TlvEncoder encoder = new TlvEncoder(256);
    int saveLength = encoder.getLength();

    // Encode backwards.
    encoder.writeOptionalBlobTlv(Tlv.ApplicationParameters, interest.getApplicationParameters().buf());
    // TODO: HopLimit.
    encoder.writeOptionalNonNegativeIntegerTlvFromDouble(Tlv.InterestLifetime,
    ...
    if (interest.getForwardingHint().size() > 0) {
      ...
      int forwardingHintSaveLength = encoder.getLength();
      encodeDelegationSet(interest.getForwardingHint(), encoder);
      encoder.writeTypeAndLength(Tlv.ForwardingHint, encoder.getLength() - forwardingHintSaveLength);
    }
  ...
  /**
   * Decode input as an Interest in NDN-TLV format v0.3 and set the fields of the
   ...
   * @throws EncodingException For invalid encoding.
   */
  private static void decodeInterestV03(Interest interest, ByteBuffer input, int[] signedPortionBeginOffset,
      int[] signedPortionEndOffset, boolean copy) throws EncodingException {
    TlvDecoder decoder = new TlvDecoder(input);
    ...
    interest
        .setApplicationParameters(new Blob(decoder.readOptionalBlobTlv(Tlv.ApplicationParameters, endOffset), copy));
