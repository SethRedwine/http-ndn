...
public class Face {
  /**
   * Create a new Face for communication with an NDN hub with the given Transport
  ...
  /**
   * Create a new Face for communication with an NDN hub at host using the default
   * port 6363 and the default TcpTransport.
   * 
   * @param host The host of the NDN hub.
   */
   ...
   * callbacks that the application gave to registerPrefix or setInterestFilter,
   * and a Data that the application gives to putData can satisfy pending
   * Interests. This way one part of an application can do Interest/Data exchange
   * with another part through the same Face. Interest loopback is disabled by
   * default.
   * 
   ...
   *                      onData.onData(interest, data) where interest is the
   *                      interest given to expressInterest and data is the
   *                      received Data object. NOTE: You must not change the
   *                      interest object - if you need to change it then make a
   *                      copy. NOTE: The library will log any exceptions thrown
   *                      by this callback, but for better error handling the
   *                      callback should catch and properly handle any
   *                      exceptions.
   * @param onTimeout     If the interest times out according to the interest
   *                      lifetime, this calls onTimeout.onTimeout(interest) where
   *                      interest is the interest given to expressInterest. If
   *                      onTimeout is null, this does not use it. NOTE: The
   *                      library will log any exceptions thrown by this callback,
   *                      but for better error handling the callback should catch
   *                      and properly handle any exceptions.
   * @param onNetworkNack When a network Nack packet for the interest is received
   *                      and onNetworkNack is not null, this calls
   *                      onNetworkNack.onNetworkNack(interest, networkNack) and
   *                      does not call onTimeout. However, if a network Nack is
   *                      received and onNetworkNack is null, do nothing and wait
   *                      for the interest to time out. (Therefore, an application
   *                      which does not yet process a network Nack reason treats
   *                      a Nack the same as a timeout.) NOTE: The library will
   *                      log any exceptions thrown by this callback, but for
   *                      better error handling the callback should catch and
   *                      properly handle any exceptions.
   * @param wireFormat    A WireFormat object used to encode the message.
   * @return The pending interest ID which can be used with removePendingInterest.
   * @throws IOException For I/O error in sending the interest.
   * @throws Error       If the encoded interest size exceeds
   *                     getMaxNdnPacketSize().
   */
  ...
  /**
   * Send the Interest through the transport, read the entire response and call
   * onData, onTimeout or onNetworkNack as described below. This uses the default
   ...
   * @param onData     When a matching data packet is received, this calls
   *                   onData.onData(interest, data) where interest is the
   *                   interest given to expressInterest and data is the received
   *                   Data object. NOTE: You must not change the interest object
   *                   - if you need to change it then make a copy. NOTE: The
   *                   library will log any exceptions thrown by this callback,
   *                   but for better error handling the callback should catch and
   *                   properly handle any exceptions.
   * @param onTimeout  If the interest times out according to the interest
   *                   lifetime, this calls onTimeout.onTimeout(interest) where
   *                   interest is the interest given to expressInterest. If
   *                   onTimeout is null, this does not use it. NOTE: The library
  ...
  /**
   * Send the Interest through the transport, read the entire response and call
   * onData or onTimeout as described below. This uses the default
   * WireFormat.getDefaultWireFormat().
   * 
   * @param interest  The Interest to send. This copies the Interest.
   * @param onData    When a matching data packet is received, this calls
   *                  onData.onData(interest, data) where interest is the interest
   *                  given to expressInterest and data is the received Data
   *                  object. NOTE: You must not change the interest object - if
   *                  you need to change it then make a copy. NOTE: The library
   *                  will log any exceptions thrown by this callback, but for
  ...
  /**
   * Send the Interest through the transport, read the entire response and call
   * onData as described below. Ignore if the interest times out.
   * 
   * @param interest   The Interest to send. This copies the Interest.
   * @param onData     When a matching data packet is received, this calls
   *                   onData.onData(interest, data) where interest is the
  ...
  /**
   * Send the Interest through the transport, read the entire response and call
   * onData as described below. Ignore if the interest times out. This uses the
  ...
  /**
   * Encode name as an Interest. If interestTemplate is not null, use its interest
   * selectors. Send the Interest through the transport, read the entire response
   * and call onData, onTimeout or onNetworkNack as described below.
   * 
   ...
   *                         template. This does not keep a pointer to the
   *                         Interest object.
   * @param onData           When a matching data packet is received, this calls
   *                         onData.onData(interest, data) where interest is the
   *                         interest given to expressInterest and data is the
   *                         received Data object. NOTE: You must not change the
   *                         interest object - if you need to change it then make
   *                         a copy. NOTE: The library will log any exceptions
   *                         thrown by this callback, but for better error
   *                         handling the callback should catch and properly
   *                         handle any exceptions.
   * @param onTimeout        If the interest times out according to the interest
   *                         lifetime, this calls onTimeout.onTimeout(interest)
   *                         where interest is the interest given to
   *                         expressInterest. If onTimeout is null, this does not
   *                         use it. NOTE: The library will log any exceptions
   *                         thrown by this callback, but for better error
   *                         handling the callback should catch and properly
   *                         handle any exceptions.
   ...
   *                         and does not call onTimeout. However, if a network
   *                         Nack is received and onNetworkNack is null, do
   *                         nothing and wait for the interest to time out.
   *                         (Therefore, an application which does not yet process
   *                         a network Nack reason treats a Nack the same as a
   *                         timeout.) NOTE: The library will log any exceptions
   *                         thrown by this callback, but for better error
   *                         handling the callback should catch and properly
   *                         handle any exceptions.
   * @param wireFormat       A WireFormat object used to encode the message.
  ...
  public long expressInterest(Name name, Interest interestTemplate, OnData onData, OnTimeout onTimeout,
    ...
    node_.expressInterest(pendingInterestId, getInterestCopy(name, interestTemplate), onData, onTimeout, onNetworkNack,
        wireFormat, this);
   ...
   * Encode name as an Interest. If interestTemplate is not null, use its interest
   * selectors. Send the Interest through the transport, read the entire response
   * and call onData, onTimeout or onNetworkNack as described below. This uses the
   * default WireFormat.getDefaultWireFormat().
   * 
   * @param name             A Name for the interest. This copies the Name.
   * @param interestTemplate If not null, copy interest selectors from the
   ...
   * Encode name as an Interest. If interestTemplate is not null, use its interest
   * selectors. Send the Interest through the transport, read the entire response
   * and call onData or onTimeout as described below.
   * 
   * @param name             A Name for the interest. This copies the Name.
   * @param interestTemplate If not null, copy interest selectors from the
   *                         template. This does not keep a pointer to the
  ...
  /**
   * Encode name as an Interest, using a default interest lifetime. Send the
   * Interest through the transport, read the entire response and call onData or
   * onTimeout as described below.
   * 
   * @param name       A Name for the interest. This copies the Name.
   * @param onData     When a matching data packet is received, this calls
   *                   onData.onData(interest, data) where interest is the
   ...
   * Encode name as an Interest. If interestTemplate is not null, use its interest
   * selectors. Send the Interest through the transport, read the entire response
   * and call onData as described below. Ignore if the interest times out.
   * 
   * @param name             A Name for the interest. This copies the Name.
   * @param interestTemplate If not null, copy interest selectors from the
   *                         template. This does not keep a pointer to the
   ...
   * Encode name as an Interest. If interestTemplate is not null, use its interest
   * selectors. Send the Interest through the transport, read the entire response
   * and call onData or onTimeout as described below. This uses the default
   * WireFormat.getDefaultWireFormat().
   * 
   * @param name             A Name for the interest. This copies the Name.
   * @param interestTemplate If not null, copy interest selectors from the
   ...
   * Encode name as an Interest. If interestTemplate is not null, use its interest
   * selectors. Send the Interest through the transport, read the entire response
   * and call onData as described below. Ignore if the interest times out. This
   ...
   * Encode name as an Interest, using a default interest lifetime. Send the
   * Interest through the transport, read the entire response and call onData or
   * onTimeout as described below. This uses the default
   * WireFormat.getDefaultWireFormat().
   * 
   * @param name      A Name for the interest. This copies the Name.
   * @param onData    When a matching data packet is received, this calls
  ...
  /**
   * Encode name as an Interest, using a default interest lifetime. Send the
   * Interest through the transport, read the entire response and call onData as
   * described below. Ignore if the interest times out.
   * 
   * @param name       A Name for the interest. This copies the Name.
   * @param onData     When a matching data packet is received, this calls
   *                   onData.onData(interest, data) where interest is the
   ...
   * Encode name as an Interest, using a default interest lifetime. Send the
   * Interest through the transport, read the entire response and call onData as
   * described below. Ignore if the interest times out. This uses the default
   * WireFormat.getDefaultWireFormat().
   * 
   * @param name   A Name for the interest. This copies the Name.
   * @param onData When a matching data packet is received, this calls
   ...
   * for registerPrefix).
   * 
   * @param keyChain        The KeyChain object for signing interests, which must
   *                        remain valid for the life of this Face. You must
   *                        create the KeyChain object and pass it in. You can
   *                        create a default KeyChain for your system with the
   *                        default KeyChain constructor.
   * @param certificateName The certificate name for signing interests. This makes
   *                        a copy of the Name. You can get the default
   ...
   * 
   * @param certificateName The certificate name for signing interest. This makes
   *                        a copy of the Name.
   */
   ...
   * @param wireFormat A WireFormat object used to encode the SignatureInfo and to
   ...
   * Append a timestamp component and a random value component to interest's name.
   * Then use the keyChain and certificateName from setCommandSigningInfo to sign
   * the interest. If the interest lifetime is not set, this sets it. Use the
   * default WireFormat to encode the SignatureInfo and to encode the interest
   * name for signing.
   * 
   * @param interest The interest whose name is appended with components.
   * @throws SecurityException If cannot find the private key for the
   ...
   * 
   * @param prefix              A Name for the prefix to register. This copies the
   *                            Name.
   * @param onInterest          If not null, this creates an interest filter from
   *                            prefix so that when an Interest is received which
   *                            matches the filter, this calls
   *                            onInterest.onInterest(prefix, interest, face,
   *                            interestFilterId, filter). The onInterest callback
   *                            should supply the Data with face.putData(). NOTE:
   *                            You must not change the prefix or filter objects -
   *                            if you need to change them then make a copy. If
   *                            onInterest is null, it is ignored and you must
   *                            call setInterestFilter. NOTE: The library will log
   *                            any exceptions thrown by this callback, but for
   *                            better error handling the callback should catch
   *                            and properly handle any exceptions.
   * @param onRegisterFailed    If register prefix fails for any reason, this
   *                            calls onRegisterFailed.onRegisterFailed(prefix).
   *                            NOTE: The library will log any exceptions thrown
   *                            by this callback, but for better error handling
   *                            the callback should catch and properly handle any
   *                            exceptions.
   * @param onRegisterSuccess   This calls
   *                            onRegisterSuccess.onRegisterSuccess(prefix,
   *                            registeredPrefixId) when this receives a success
   *                            message from the forwarder. If onRegisterSuccess
   *                            is null, this does not use it. (The
   *                            onRegisterSuccess parameter comes after
   *                            onRegisterFailed because it can be null or
   *                            omitted, unlike onRegisterFailed.) NOTE: The
   *                            library will log any exceptions thrown by this
   *                            callback, but for better error handling the
   *                            callback should catch and properly handle any
   *                            exceptions.
   * @param registrationOptions The registration options for finer control of how
   *                            to forward an interest and other options.
   * @param wireFormat          A WireFormat object used to encode the message.
  ...
  public long registerPrefix(Name prefix, OnInterestCallback onInterest, OnRegisterFailed onRegisterFailed,
      OnRegisterSuccess onRegisterSuccess, RegistrationOptions registrationOptions, WireFormat wireFormat)
      throws IOException, SecurityException {
    // Get the registeredPrefixId now so we can return it to the caller.
    long registeredPrefixId = node_.getNextEntryId();

    node_.registerPrefix(registeredPrefixId, prefix, onInterest, onRegisterFailed, onRegisterSuccess,
        registrationOptions, wireFormat, commandKeyChain_, commandCertificateName_, this);
   ...
   *             onRegisterSuccess, registrationOptions, wireFormat) where the
  ...
  public long registerPrefix(Name prefix, OnInterestCallback onInterest, OnRegisterSuccess onRegisterSuccess,
      OnRegisterFailed onRegisterFailed, RegistrationOptions registrationOptions, WireFormat wireFormat)
      throws IOException, SecurityException {
    return registerPrefix(prefix, onInterest, onRegisterFailed, onRegisterSuccess, registrationOptions, wireFormat);
  }
   ...
   */
  public long registerPrefix(Name prefix, OnInterestCallback onInterest, OnRegisterFailed onRegisterFailed,
      OnRegisterSuccess onRegisterSuccess, RegistrationOptions registrationOptions)
      throws IOException, SecurityException {
    return registerPrefix(prefix, onInterest, onRegisterFailed, onRegisterSuccess, registrationOptions,
        WireFormat.getDefaultWireFormat());
  }

  /**
   * @deprecated Use registerPrefix(prefix, onInterest, onRegisterFailed,
   *             onRegisterSuccess, registrationOptions) where the
   *             onRegisterSuccess parameter comes after onRegisterFailed.
   */
  public long registerPrefix(Name prefix, OnInterestCallback onInterest, OnRegisterSuccess onRegisterSuccess,
  ...
  /**
   * Register prefix with the connected NDN hub and call onInterest when a
   * matching interest is received. Use default RegistrationOptions.
   * 
   * @param prefix            A Name for the prefix to register. This copies the
   *                          Name.
   * @param onInterest        If not null, this creates an interest filter from
   ...
   *                          You must not change the prefix or filter objects -
   *                          if you need to change them then make a copy. If
   *                          onInterest is null, it is ignored and you must call
   *                          setInterestFilter. NOTE: The library will log any
   *                          exceptions thrown by this callback, but for better
   *                          error handling the callback should catch and
   *                          properly handle any exceptions.
   * @param onRegisterFailed  If register prefix fails for any reason, this calls
   *                          onRegisterFailed.onRegisterFailed(prefix). NOTE: The
   *                          library will log any exceptions thrown by this
   *                          callback, but for better error handling the callback
   *                          should catch and properly handle any exceptions.
   * @param onRegisterSuccess This calls
   *                          onRegisterSuccess.onRegisterSuccess(prefix,
   *                          registeredPrefixId) when this receives a success
   *                          message from the forwarder. If onRegisterSuccess is
   *                          null, this does not use it. (The onRegisterSuccess
   *                          parameter comes after onRegisterFailed because it
   *                          can be null or omitted, unlike onRegisterFailed.)
   *                          NOTE: The library will log any exceptions thrown by
   *                          this callback, but for better error handling the
   *                          callback should catch and properly handle any
   *                          exceptions.
   * @param wireFormat        A WireFormat object used to encode the message.
   * @return The registered prefix ID which can be used with
  ...
  public long registerPrefix(Name prefix, OnInterestCallback onInterest, OnRegisterFailed onRegisterFailed,
    ...
    return registerPrefix(prefix, onInterest, onRegisterFailed, onRegisterSuccess, new RegistrationOptions(),
        wireFormat);
  }
   ...
   * Register prefix with the connected NDN hub and call onInterest when a
   * matching interest is received. This uses the default
   * WireFormat.getDefaultWireFormat(). Use default RegistrationOptions.
   * 
   * @param prefix            A Name for the prefix to register. This copies the
   *                          Name.
   * @param onInterest        If not null, this creates an interest filter from
  ...
  /**
   * @deprecated Use registerPrefix(prefix, onInterest, onRegisterFailed,
   *             onRegisterSuccess) where the onRegisterSuccess parameter comes
   *             after onRegisterFailed.
   */
  public long registerPrefix(Name prefix, OnInterestCallback onInterest, OnRegisterSuccess onRegisterSuccess,
   ...
   */
  public long registerPrefix(Name prefix, OnInterestCallback onInterest, OnRegisterFailed onRegisterFailed,
      RegistrationOptions registrationOptions, WireFormat wireFormat) throws IOException, SecurityException {
    return registerPrefix(prefix, onInterest, onRegisterFailed, null, registrationOptions, wireFormat);
  }

  /**
   * Register prefix with the connected NDN hub and call onInterest when a
   * matching interest is received. To register a prefix with NFD, you must first
   * call setCommandSigningInfo. This uses the default
   * WireFormat.getDefaultWireFormat().
   * 
   * @param prefix              A Name for the prefix to register. This copies the
   *                            Name.
  ...
  public long registerPrefix(Name prefix, OnInterestCallback onInterest, OnRegisterFailed onRegisterFailed,
    ...
    return registerPrefix(prefix, onInterest, onRegisterFailed, null, registrationOptions,
        WireFormat.getDefaultWireFormat());
  }
   ...
   * Register prefix with the connected NDN hub and call onInterest when a
   * matching interest is received. To register a prefix with NFD, you must first
   * call setCommandSigningInfo. Use default RegistrationOptions.
   * 
   * @param prefix           A Name for the prefix to register. This copies the
   *                         Name.
   * @param onInterest       If not null, this creates an interest filter from
   ...
   *                         onInterest.onInterest(prefix, interest, face,
   *                         interestFilterId, filter). The onInterest callback
   *                         should supply the Data with face.putData(). NOTE: You
   *                         must not change the prefix or filter objects - if you
   *                         need to change them then make a copy. If onInterest
   *                         is null, it is ignored and you must call
   *                         setInterestFilter. NOTE: The library will log any
   *                         exceptions thrown by this callback, but for better
   *                         error handling the callback should catch and properly
   *                         handle any exceptions.
   * @param onRegisterFailed If register prefix fails for any reason, this calls
   *                         onRegisterFailed.onRegisterFailed(prefix). NOTE: The
   *                         library will log any exceptions thrown by this
  ...
  public long registerPrefix(Name prefix, OnInterestCallback onInterest, OnRegisterFailed onRegisterFailed,
    ...
    return registerPrefix(prefix, onInterest, onRegisterFailed, null, new RegistrationOptions(), wireFormat);
  }
  ...
  public long registerPrefix(Name prefix, OnInterestCallback onInterest, OnRegisterFailed onRegisterFailed)
      throws IOException, SecurityException {
    return registerPrefix(prefix, onInterest, onRegisterFailed, null, new RegistrationOptions(),
        WireFormat.getDefaultWireFormat());
  }
   ...
   *                   used to match the name of an incoming Interest. This makes
   *                   a copy of filter.
   * @param onInterest When an Interest is received which matches the filter, this
   ...
   *                   Interest.
   * @param onInterest This creates an interest filter from prefix so that when an
  ...
  /**
   * The OnInterestCallback calls this to put a Data packet which satisfies an
   * Interest. This uses the default WireFormat.getDefaultWireFormat() to encode
   * data.
   * 
   * @param data The Data packet which satisfies the interest.
   * @throws Error If the encoded Data packet size exceeds getMaxNdnPacketSize().
  ...
  /**
   * Process any packets to receive and call callbacks such as onData, onInterest
   * or onTimeout. This returns immediately if there is no data to receive. This
   * blocks while calling the callbacks. You should repeatedly call this from an
   * event loop, with calls to sleep as needed so that the loop doesn’t use 100%
   * of the CPU. Since processEvents modifies the pending interest table, your
   * application should make sure that it calls processEvents in the same thread
   * as expressInterest (which also modifies the pending interest table). This may
   * throw an exception for reading data or in the callback for processing the
   * data. If you call this from an main event loop, you may want to catch and
  ...
  /**
   * Call callback.run() after the given delay. Even though this is public, it is
   * not part of the public API of Face. This default implementation just calls
   * Node.callLater, but a subclass can override.
   * 
