...
public class Interest implements ChangeCountable {
   ...
   * date. The application should call this before creating any Interest (even to
   * set the default again to true), or the application should explicitly call
  ...
  public final SignedBlob wireEncode(WireFormat wireFormat) {
    ...
    Blob encoding = Tlv0_2WireFormat.get().encodeInterest(this, signedPortionBeginOffset, signedPortionEndOffset);
  ...
  private void wireDecodeHelper(ByteBuffer input, WireFormat wireFormat, boolean copy) throws EncodingException {
    ...
    if (wireFormat == WireFormat.getDefaultWireFormat())
      // This is the default wire encoding.
      setDefaultWireEncoding(new SignedBlob(input, copy, signedPortionBeginOffset[0], signedPortionEndOffset[0]),
          WireFormat.getDefaultWireFormat());
    else
  ...
  public final long getIncomingFaceId() {
    IncomingFaceId field = lpPacket_ == null ? null : IncomingFaceId.getFirstHeader(lpPacket_);
   ...
   *                       This makes a copy of the object. If no forwarding hint
   *                       is specified, set to a new default DelegationSet() with
   *                       no entries.
   * @return This Interest so that you can chain calls to update values.
   */
  public final Interest setForwardingHint(DelegationSet forwardingHint) {
    forwardingHint_.set(forwardingHint == null ? new DelegationSet() : new DelegationSet(forwardingHint));
    ++changeCount_;
    return this;
  }
   ...
   * @return This Interest so that you can chain calls to update values.
   */
  public final Interest setApplicationParameters(Blob applicationParameters) {
    applicationParameters_ = (applicationParameters == null ? new Blob() : applicationParameters);
    ++changeCount_;
    return this;
  }
   ...
   */
  public final Interest setParameters(Blob applicationParameters) {
  ...
  public final Interest appendParametersDigestToName() {
    ...
    try {
      getName().appendParametersSha256Digest(new Blob(Common.digestSha256(applicationParameters_.buf()), false));
   ...
   */
  public final boolean matchesData(Data data, WireFormat wireFormat) {
    ...
    int minSuffixComponents = hasMinSuffixComponents ? getMinSuffixComponents() : 0;
    ...
    if (getExclude().size() > 0 && fullNameLength > interestNameLength) {
      if (interestNameLength == fullNameLength - 1) {
        ...
        if (getExclude().matches(data.getFullName(wireFormat).get(interestNameLength)))
          return false;
      } else {
   ...
   * @return True if the given Data packet can satisfy this Interest.
   */
  public final boolean matchesData(Data data) {
   ...
   */
  public boolean getDidSetCanBePrefix_() {
